...
let format_date_dmy d : string =
  try
    ...
    ^ string_of_int d.tm_year
  with _ -> raise (Failure "Could not convert string to int")

let format_date d : string =
  try
    string_of_int d.tm_mon ^ "/" ^ string_of_int d.tm_mday ^ "/"
    ^ string_of_int d.tm_year ^ " " ^ string_of_int d.tm_hour ^ ":"
    ^ string_of_int d.tm_min ^ ":" ^ string_of_int d.tm_sec
  with _ -> raise (Failure "Could not convert string to int")
...
let extractDate d =
  ...
  let dlist_int =
    try List.map int_of_string dlist
    with _ -> raise (Failure ("Could not parse: " ^ List.hd dlist))
  in
  ...
  | m :: d :: _ ->
      ( {
          tm_mon = m;
          tm_mday = d;
          tm_year = (localtime (gettimeofday ())).tm_year + 1900;
          tm_hour = 0;
          tm_min = 0;
          tm_sec = 0;
          tm_wday = 0;
...
let getDate : tm =
  ...
  {
    current_tm with
    tm_year = current_tm.tm_year + 1900;
    tm_mon = current_tm.tm_mon + 1;
  }
...
let rec process_string_list (slst : string list) (acc : string list)
    (llst : lumber list) : lumber list =
  match slst with
  | "" :: t ->
      if acc == [] then process_string_list t [] llst
      else process_string_list t [] (convert_to_lumber acc :: llst)
  | x :: t -> process_string_list t (acc @ [ x ]) llst
  | _ -> (
      if acc == [] then llst
      else try convert_to_lumber acc :: llst with _ -> [] )

let txtToLumberList txt = List.rev (process_string_list (read_lines txt) [] [])
